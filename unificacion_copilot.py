"""
Unificación - Sistema de Gestión de Stock y Cálculo de Etiquetas
==================================================================

Este módulo integra datos de stock, pedidos y componentes para generar
tablas resumen con cálculos optimizados de cilindros y metraje.
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('unificacion.log', encoding='utf-8')
    ]
)
...
def obtener_ruta_salida() -> str:
    ...
    Returns:
        str: Ruta absoluta donde guardar los archivos
    """
    try:
        ...
        logger.warning(f"No se pudo determinar ruta del script: {e}")
    ...
    try:
        ...
        logger.warning(f"No se pudo acceder a carpeta Descargas: {e}")
    ...
    try:
        ...
        logger.warning(f"No se pudo acceder a carpeta temporal: {e}")
...
FACTOR_CILINDRO_A_MM: float = 3.175  # Z a milímetros
ROLLO_BASE_MM: float = 330  # Rollo estándar 330 mm
BANDERA_HORIZONTAL: float = 20  # mm de bandera (se restan del ancho útil)
GAP_HORIZONTAL_OBJETIVO: float = 2.7  # Gap objetivo horizontal
GAP_HORIZONTAL_MIN: float = 2.3  # Gap horizontal mínimo permitido
GAP_HORIZONTAL_MAX: float = 20.0  # Gap horizontal máximo permitido
GAP_VERTICAL_OBJETIVO: float = 2.7  # Gap objetivo vertical
GAP_VERTICAL_MIN: float = 2.5  # Gap vertical mínimo permitido
GAP_VERTICAL_MAX: float = 20.0  # Gap vertical máximo permitido
MAX_REPETICIONES_VERTICALES: int = 8  # Máximo número de repeticiones verticales
AJUSTE_DESARROLLO_REP_MM: float = 0.75  # mm adicionales por repeat (merma)

# Conjuntos de cilindros disponibles (ordenados de mayor a menor)
...
class EvaluacionZUniforme:
    """
    Resultado de evaluación de un Z (cilindro) particular.
    
    Attributes:
        z: Número del cilindro (60-168)
        desarrollo_mm: Desarrollo del cilindro en mm
        n: Número de etiquetas que caben verticalmente
        gap: Gap vertical resultante en mm
        diff_obj: Diferencia con el gap objetivo
        valido: Si la configuración es válida
        detalle: Detalles adicionales o razón de invalidez
    """
    def __init__(self, z: int, desarrollo_mm: float, n: int, gap: float,
                 diff_obj: float, valido: bool, detalle: str):
        self.z = z
...
def evaluar_z_uniforme(z: int, alto_etq: float, gap_obj: float,
    ...
    Args:
        ...
        gap_obj: Gap objetivo en mm (típicamente 2.7)
        gap_min: Gap mínimo permitido (típicamente 2.5)
        gap_max: Gap máximo permitido (típicamente 20)
        max_n: Máximo número de repeticiones verticales
    
    Returns:
        EvaluacionZUniforme: Objeto con n (repeticiones), gap, y validez
    """
...
def crear_tabla_resumen(df_pedidos_componentes_stock, df_pedidos_pendientes=None, df_listademateriales=None):
    ...
    for componente_unico in df_resumen['Componente'].unique():
        ...
        if len(filas_componente) > 0:
            ...
            if stock_inicial < 0:
                logger.warning(f" Stock inicial negativo para componente {componente_unico}: {stock_inicial:.4f} → Reemplazando con 0")
                df_resumen.at[idx_primera, 'StockAcumulado'] = 0
                negativos_iniciales += 1
    
    if negativos_iniciales > 0:
        logger.info(f" Se corrigieron {negativos_iniciales} componentes con stock inicial NEGATIVO")
    ...
    logger.info("\n[DEBUG] VERIFICACION - Factor_unificado (NUEVA REGLA)")
    logger.info("─" * 100)
    logger.info(f"Cantidad (min): {df_resumen['Cantidad'].min()} | (max): {df_resumen['Cantidad'].max()}")
    logger.info(f"Unidad_medida UNIQUE: {df_resumen['Unidad_medida'].unique()}")
    logger.info("\nPrimeros 10 registros:")
    logger.info(str(df_resumen[['Cantidad', 'Unidad_medida']].head(10).to_string()))
    logger.info("─" * 100)
    ...
    logger.info("\n[DEBUG] RESULTADO - Factor_unificado calculado (NUEVA REGLA)")
    logger.info("─" * 100)
    logger.info(str(df_resumen[['Cantidad', 'Unidad_medida', 'Factor_unificado']].head(10).to_string()))
    logger.info("─" * 100)
    ...
    logger.info("\n[DEBUG] NORMALIZACIÓN DE STOCK POR UNIDAD_MEDIDA")
    logger.info("─" * 100)
    ...
    logger.info("  ✓ Stock normalizado según Unidad_Medida")
    logger.info(str(df_resumen[['Componente', 'Unidad_medida', 'Factor_unificado', 'StockAcumulado']].drop_duplicates(subset=['Componente']).head(10).to_string()))
    logger.info("─" * 100)
...
def obtener_credenciales_bd() -> Dict[str, str]:
    """
    Obtiene credenciales de base de datos de variables de entorno o valores por defecto.
    ...
    return {
        ...
        'username': os.getenv('DB_USERNAME', 'ReportesInnoprint'),
        'password': os.getenv('DB_PASSWORD', 'm^9S*^N$v2AR')
    }

# Obtener configuración de conexión
db_credentials = obtener_credenciales_bd()
...
def crear_engine_bd(conn_str: str) -> Optional[Engine]:
    """
    Crea y retorna un engine de SQLAlchemy configurado.
    
    Args:
        conn_str: Cadena de conexión a la base de datos
    
    Returns:
        Engine de SQLAlchemy o None si falla la conexión
    """
    try:
        logger.info("Creando conexión optimizada a SQL Server...")
        ...
        logger.error("Verifica que pyodbc esté instalado: pip install pyodbc")
        return None

try:
    engine = crear_engine_bd(conn_str)
    
    if engine is None:
        logger.error("No se pudo crear el engine de base de datos")
        sys.exit(1)
    
    logger.info("Conectando a SQL Server y ejecutando SELECT FROM vista_stock_General...")
    ...
    logger.info(f" Datos de stock cargados ✓ ({len(df_stock)} registros)")

    logger.info("[INFO] Cargando lista de materiales...")
    ...
    """
    logger.info("[DEBUG] Ejecutando query de lista de materiales...")
    df_listademateriales = pd.read_sql(query_listademateriales, engine)
    logger.info(f" Lista de materiales cargada ({len(df_listademateriales)} registros) ✓")
    ...
    logger.info("\n[DEBUG] CALCULANDO STOCK_FINAL EN DF_STOCK")
    logger.info("─" * 100)
    
    df_stock_con_unidad['Stock_Final'] = df_stock_con_unidad.apply(calcular_stock_final, axis=1)
    
    logger.info("  ✓ Stock_Final calculado según Unidad_Medida")
    logger.info(str(df_stock_con_unidad[['ItemCode', 'Unidad_medida', 'Cantidad', 'Stock_Final']].drop_duplicates(subset=['ItemCode']).head(10).to_string()))
    logger.info("─" * 100)
    
    # Actualizar df_stock con la nueva columna (usar Stock_Final en lugar de Cantidad)
    df_stock = df_stock_con_unidad.copy()

    logger.info("[INFO] Cargando pedidos pendientes...")
    try:
        ...
        logger.info("[OK] Pedidos pendientes cargados")
    except Exception as e:
        logger.warning(f" No se pudieron cargar pedidos pendientes: {e}")
        logger.info("[INFO] Continuando con DataFrame vacío...")
    ...
    if negativos_antes > 0:
        logger.info(f"\n[ADVERTENCIA] Se encontraron {negativos_antes} componentes con StockAcumulado NEGATIVO")
        logger.info("[INFO] Reemplazando valores negativos con 0")
    ...
    logger.info("Ejecutando motor de cálculo para obtener Z sugerido y metraje...")
    ...
    logger.info("Columnas del Motor de Calculo:")
    logger.info("  - Z_Sugerido: Cilindro elegido (60-168) para optimizar metraje")
    logger.info("  - Desarrollo_mm: Perimetro del cilindro en mm (Z × 3.175)")
    logger.info("  - Ancho_rollo_mm: Ancho estandar del rollo (330 mm)")
    logger.info("  - n_vertical: Etiquetas que caben verticalmente en el cilindro")
    logger.info("  - etq_eje_horizontal: Etiquetas que caben horizontalmente en el rollo")
    logger.info("  - Etiquetas_repeticion: Etiquetas por repeticion (n_vertical × etq_eje_horizontal)")
    logger.info("  - Repeticiones: Vueltas del cilindro necesarias")
    logger.info("  - Merma_mm: Merma total en milimetros (0.75 mm × Repeticiones)")
    logger.info("  - Merma_m: Merma total en metros (Merma_mm / 1000)")
    logger.info("  - ML_sin_merma: Metros lineales de material (sin contar merma)")
    logger.info("  - M2_sin_merma: Metros cuadrados de material (sin contar merma)")
    logger.info("=" * 50)
    
    # Mostrar resultados del motor de cálculo
    logger.info("\n=== RESULTADOS DEL MOTOR DE CÁLCULO ===")
    logger.info(f"Total de registros procesados: {len(df_pedidos_componentes_stock)}")
    logger.info("\nPrimeros 15 registros con valores calculados:")
    ...
    logger.info(str(resultado_motor.to_string(index=False)))
    
    logger.info("\n" + "=" * 50)
    logger.info("Estadísticas del Motor:")
    logger.info(f"  - Registros con Z calculado: {df_pedidos_componentes_stock['Z_Sugerido'].notna().sum()}")
    logger.info(f"  - ML promedio: {df_pedidos_componentes_stock['ML_sin_merma'].mean():.2f} m")
    logger.info(f"  - M2 promedio: {df_pedidos_componentes_stock['M2_sin_merma'].mean():.2f} m²")
    logger.info(f"  - ML total: {df_pedidos_componentes_stock['ML_sin_merma'].sum():.2f} m")
    logger.info(f"  - M2 total: {df_pedidos_componentes_stock['M2_sin_merma'].sum():.2f} m²")
    merma_total_m = df_pedidos_componentes_stock['Merma_m'].sum()
    merma_total_mm = df_pedidos_componentes_stock['Merma_mm'].sum()
    logger.info(f"  - Merma total: {merma_total_mm:.2f} mm = {merma_total_m:.2f} m")
    logger.info("=" * 50)

except Exception as e:
    logger.error(f"Error en la conexión o consulta: {e}")
    logger.error("INSTALACIÓN FALTANTE: Por favor instala pyodbc:")
    logger.error("   pip install pyodbc")
    sys.exit(1)

# Cerrar conexión de BD de forma segura
try:
    engine.dispose()
...
def aplicar_colores_disponibilidad(workbook_path: str, sheet_name: str = 'TablaResumen') -> None:
    """
    Aplica formato condicional de colores a las columnas Disponibilidad y Disponibilidad_Factor.
    
    Verde para 1 (disponible), rojo para 0 (no disponible).
    ...
    try:
        ...
        logger.info(f"Colores condicionales aplicados a {sheet_name} ✓")
    except Exception as e:
        logger.warning(f"No se pudieron aplicar colores: {e}")

def guardar_excel(filename: str, dataframes: Dict[str, pd.DataFrame]) -> bool:
    """
    Guarda múltiples dataframes en un archivo Excel.
    
    Args:
        filename: Nombre del archivo Excel
        dataframes: Diccionario de {nombre_hoja: dataframe}
...
try:
    # Preparar dataframes a guardar
    dataframes = {
        'StockGeneral': df_stock,
        'ListaMateriales': df_listademateriales,
        'StockAcumulado': df_stock_acumulado,
        'TablaResumen': df_resumen,
        'PedidosComponentesStock': df_pedidos_componentes_stock
    }
    
    # Intentar guardar archivo principal
    if guardar_excel(excel_filename, dataframes):
        aplicar_colores_disponibilidad(excel_filename, 'TablaResumen')
        logger.info(f"Archivo guardado exitosamente: {excel_filename} ✓")
    else:
        raise PermissionError("No se pudo guardar el archivo principal")
    
except PermissionError:
    # Si el archivo está bloqueado, usar nombre temporal
    excel_filename = os.path.join(RUTA_SALIDA, 'tablas_unificadas_temp.xlsx')
    logger.warning(f"Archivo principal bloqueado, usando nombre alternativo...")
    
    if guardar_excel(excel_filename, dataframes):
        aplicar_colores_disponibilidad(excel_filename, 'TablaResumen')
        logger.warning(f"Archivo guardado como temporal: {excel_filename}")
    else:
        logger.error("No se pudo guardar el archivo con nombre temporal")

except Exception as e:
    logger.error(f"Error inesperado al guardar archivo Excel: {e}")
    logger.error("El proceso continuará pero no se guardó el archivo Excel")
...
logger.info("=" * 70)
logger.info(f"Archivo generado: {excel_filename}")
logger.info("Cerrando aplicación en 5 segundos...")

for i in range(5, 0, -1):
    logger.info(f"Cerrando en {i}...")
    time.sleep(1)

logger.info("¡Listo! Aplicación finalizada correctamente.")

# ======================================================================================
# NOTAS DE EJECUCIÓN
# ======================================================================================
# Para ejecutar este script desde PowerShell o cmd:
# python "ruta/completa/al/archivo/unificacion_copilot.py"
#
# Variables de entorno opcionales para configurar BD:
# set DB_SERVER=tu_servidor
